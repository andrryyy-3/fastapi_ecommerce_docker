...
class ProductCreate(BaseModel):
    """
        ...
        Используется в POST и PUT запросах.
    """
    name: str = Field(min_length=3, max_length=50, description="Название товара (3-100 символов)")
    ...
    category_id: int = Field(ge=0, description="ID категории, к которой относится товар")
...
class Review(BaseModel):
    ...
    comment_date: datetime = Field(default=datetime.now, description="Поле создания отзыва")
...
class ReviewCreate(BaseModel):
    """
        ...
        Используется в POST и PUT запросах.
    """
    product_id: int = Field(description="Уникальный идентификатор отзыва")
    comment: Optional[str] = Field(None, description="Текстовое поле для отзыва")
    grade: int = Field(ge=1, le=5, description="Оценка от 1 до 5")
